...
The current version is **1.2.11** (5 June, 2018).
...
* `target/jpmml-sparkml-executable-1.2-SNAPSHOT.jar` - Example application JAR file.
...
```
spark-submit --master local --class org.jpmml.sparkml.Main target/jpmml-sparkml-executable-1.2-SNAPSHOT.jar --schema-input src/test/resources/schema/Iris.json --pipeline-input src/test/resources/pipeline/DecisionTreeIris.zip --pmml-output DecisionTreeIris.pmml
```

Getting help:
```
spark-submit --master local --class org.jpmml.sparkml.Main target/jpmml-sparkml-executable-1.2-SNAPSHOT.jar --help
```
