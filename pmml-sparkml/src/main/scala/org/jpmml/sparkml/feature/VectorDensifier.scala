...
class VectorDensifier(override val uid: String) extends Transformer with HasInputCol with HasOutputCol with DefaultParamsWritable {
	...
	def this() = this(Identifiable.randomUID("vecDensifier"))
	...
	def transform(dataset: Dataset[_]): Dataset[Row] = {
		...
		dataset.withColumn(outputColName, toDenseUDF(dataset(inputColName)))
	}
}

object VectorDensifier extends DefaultParamsReadable[VectorDensifier] {

	def toDense(vec: Vector): DenseVector = {
		if(vec != null){
			vec match {
